function MultiSelectTag(e, t = {
    ...
    n = document.getElementById(e),
        function() {
            ...
            t.shadow && p.classList.add("shadow"), t.rounded && p.classList.add("rounded"), 
            p.append(c), 
    ...
    butt = document.createElement('span')
    butt.innerText = "Select All"
    butt.classList.add("btn")
    butt.classList.add("btn-primary")
    butt.classList.add("selectAll")
    document.querySelector(".input-body").appendChild(butt)


    document.querySelector('.selectAll').addEventListener('click', function(){

        document.querySelector("div.body").querySelector(".input-container").innerHTML = ""
        document.querySelector(".drawer").querySelector("ul").innerHTML = ''

        let ele = d;

        let fullList = document.querySelector(".formContainer form select").children
        
        for(let i=0; i<fullList.length; i++){
            fullList[i].selected = true;
            f(ele[i])
        }
